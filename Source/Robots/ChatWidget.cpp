...
void SChatWidget::Construct(const FArguments& InArgs)
{
	...
	ChildSlot // Build the base for the chatbox
		...
		.OnGenerateRow(this, &SChatWidget::OnGenerateRowForList) // The widget is trying to draw, give the elements
	...
	+ SVerticalBox::Slot()
		...
		[
			...
			.OnTextCommitted(this, &SChatWidget::OnChatTextCommitted) // function to call when text is entered
...
void SChatWidget::OnChatTextChanged(const FText& InText) // Called everytime the user presses a key on the input bar
{
...
void SChatWidget::OnChatTextCommitted(const FText& InText, ETextCommit::Type CommitMethod) // The chat box is submitted
{
	...
	if (ChatInput.IsValid())
	{
		...
		if (!NFText.IsEmpty())
		{
			ARobotPlayerState* MyPS = Cast<ARobotPlayerState>(OwnerHUD->MyPC->PlayerState); // cast to our player state that contains the rpc functions
			if (MyPS)
			{
...
void SChatWidget::Tick(const FGeometry& AllottedGeometry, const double InCurrentTime, const float InDeltaTime) // called everyframe and used for our gamelogic
{
